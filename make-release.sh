...
if ! grep -q "value: quay.io/eclipse/che-dashboard:$RELEASE" $filename; then
  ...
  echo "7.30.0" > /tmp/che.properties # todo
...
releaseOperatorCode() {
  ...
  docker buildx build --build-arg DEV_WORKSPACE_CONTROLLER_VERSION=${DEV_WORKSPACE_CONTROLLER_VERSION} --build-arg DEV_WORKSPACE_CHE_OPERATOR_VERSION=${DEV_WORKSPACE_CHE_OPERATOR_VERSION} --platform "$BUILDX_PLATFORMS" --push -t "quay.io/aandriienko/che-operator:${RELEASE}" .
}
